...
class JobDashBoardSummary extends Component {
  ...
  render() {
    ...
    let { user : {editing, loading} ,jobs: {jobId ,job, jobSummary, company , location,state,salary,salaryFreq, role, additionalInfo, tradeClassificaiton,imageUrl, workType, createdAt} } = this.props;
    ...
    return (
      <div className="jobDashCard">
          ...
          <img
            ...
            data-tip="Edit Job"
            data-place="bottom"
            onClick={() => { if (window.confirm(`Are you sure you wish to edit ${job}`)) this.props.editJob(jobId, true, this.props.history) } }
          ></img>
          <ReactTooltip />
            ...
            data-tip="Delete Job"
            data-place="bottom"
            onClick={() => { if (window.confirm(`Are you sure you wish to delete ${job}`)) this.props.deleteJob(jobId, userId, this.props.history )} }
          ></img>
          <ReactTooltip />
        ...
        <p>Expiring {dayjs(createdAt).to(dayjs(createdAt).add(30,'days'))}
            </p>
            <p>View Counts</p>
            <p>Applicants</p>
      </div>
