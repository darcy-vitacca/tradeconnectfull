...
if (props.inboxAll){
    ...
    return props.inboxAll.map((val, idx) => {
        ...
        return (
          <tr data-id={idx} key={uuid()} >
            <td data-id={idx} onClick={() => {props.view(val)}}>{val.senderHandle}</td>
            <td data-id={idx} onClick={() => {props.view(val)}}>{val.subject}</td>
            <td data-id={idx} onClick={() => {props.view(val)}}>{dayjs(val.createdAt).fromNow()}</td>
            <td data-id={idx}>
              <img
                data-id={idx}
  ...
  console.log("here")
    return props.sentAll.map((val, idx) => {
        ...
        return (
          <tr data-id={idx} key={uuid()}>
            <td data-id={idx} onClick={() => {props.view(val)}} >{val.recipientHandle}</td>
            <td data-id={idx} onClick={() => {props.view(val)}}>{val.subject}</td>
            <td data-id={idx} onClick={() => {props.view(val)}}>{dayjs(val.createdAt).fromNow()}</td>
            <td data-id={idx}>
              <img
